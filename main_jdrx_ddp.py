...
def main():
    ...
    if method =='jdrx':
        ...
        run_name = f"{method}_ddp{int(os.environ.get('WORLD_SIZE', 1))}_bs{args.batch_size}_dim{feat_dim}_ep{args.epochs}_subsize{args.num_subsets}"
        args.out_dir = os.path.join(args.out_dir, run_name)
    else:
        feat_dim = int(args.projector.split("-")[-1])  # last projector dim
        run_name = f"{method}_ddp{int(os.environ.get('WORLD_SIZE', 1))}_bs{args.batch_size}_dim{feat_dim}_ep{args.epochs}"
        args.out_dir = os.path.join(args.out_dir, run_name)
    ...
    if is_distributed:
        ddp_setup("nccl")   # 只负责 init_process_group 等
